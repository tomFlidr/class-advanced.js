...
Class = (function (_globalScope) {
	...
	if ('Class' in _globalScope) return _globalScope['Class'];
	...
	*/
	...
	$class._extendParentProtoGetProtectedPublicProxyCall = function (_dynamicName, _targetFn, _staticCalls) {
		var _staticStr = $class._constants['static'],
			_selfStr = $class._constants['self'],
			_extendStr = $class._constants['Extend'];
		if ($class._editorIntelliSense) return _targetFn;
		return function () {
			var _currentSelf = this[_selfStr],
				_result = _targetFn.apply(
					_staticCalls ? this[_staticStr] : this, $class._normalizeArgs(arguments)
				);
	...
	$class._extendParentProtoGetPrivateProxyCall = function (_dynamicName, _targetFn, _parentClassImprint, _staticCalls) {
		var _inherited = $class._constants['Inherited'],
			_targetImprint = '';
		...
		var _proxyFn = $class._extendParentProtoGetPrivateProxyCallProvider(
			_dynamicName, _targetFn, _targetImprint, _staticCalls
		);
	...
	$class._extendParentProtoGetPrivateProxyCallProvider = function (_dynamicName, _targetFn, _targetImprint, _staticCalls) {
		var _selfStr = $class._constants['self'],
			_classImprint = $class._constants['GetClassUid'],
		...
		return function () {
			...
			if (_self[_classImprint]() != _targetImprint) {
				throw new Error(
					"Private method call: '" + _dynamicName
						+ "' not allowed from class context: '"
						+ _self[_fullname] + "'."
				);
			}
			return _targetFn.apply(_staticCalls ? _self : this, $class._normalizeArgs(arguments));
		}
	};

	/**
	 * @summary								Define system 'Name' property on each dynamic function if necessary.
	 * @access	private
	...
	$class._declareCurrentPrototype = function (classDefinition, cfg) {
		...
		var _cfgExtend = cfg[$class._constants['Extend']];
	...
	$class._isArgs = function (obj) {
		// be carefull for WSH - thee is no Arguments typed array, there is only Object with length... but no a string, just object returned by typeof()
		return typeof(obj) == 'object' && typeof(obj['length']) == 'number' && Object['prototype']['toString']['apply'](obj) != '[object Array]'
	};
